...
このリポジトリに含まれている GitHub Actions のフックは `.github/workflows/main.yml` で定義されており、以下の 2 つです。

- `push` to `main`
- `pull_request` to `main`

なお、このリポジトリ内には Production 専用のデプロイ job は含まれていません。
そのため、Production への反映タイミングを確認したい場合は、まず `main` ブランチへの push（または PR の merge による push）が起点になっているかを確認してください。

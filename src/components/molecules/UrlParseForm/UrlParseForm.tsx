...
class UrlParseForm extends React.Component<{}, State> {
  ...
  urlParamsTbodyTr (): ReactNode {
    return this.state.urlParams.map((urlParam: UrlParam, i: number) => {
      return (
                <tr key={i}>
                    <td><input type={'text'} className={'form-control'} value={urlParam.key} data-index={i} data-type={'key'} onChange={this.onChangeParamsText} /></td>
                    <td><input type={'text'} className={'form-control'} value={urlParam.value} data-index={i} data-type={'value'} onChange={this.onChangeParamsText} /></td>
  ...
  onChangeParsedURLText (e: React.ChangeEvent<HTMLInputElement>): void {
    this.setState({
      parsedUrlText: e.target.value
    })
  }

  onChangeBaseURLText (e: React.ChangeEvent<HTMLInputElement>): void {
    this.setState({
      baseUrlText: e.target.value
    })
  }

  onChangeParamsText (e: React.ChangeEvent<HTMLInputElement>): void {
  ...
  onClickUrlParse (e: React.MouseEvent<HTMLElement>): void {
  ...
  onClickUrlBuild (e: React.MouseEvent<HTMLElement>): void {
  ...
  onClickDeleteParam (e: React.MouseEvent<HTMLElement>): void {
    const clickButtonIndex = e.currentTarget.getAttribute('data-index')
    const newUrlParams = this.state.urlParams.filter((_: UrlParam, index: number) => clickButtonIndex !== index.toString())
    this.setState({
      urlParams: newUrlParams
    })
  }

  onClickAddParam (e: React.MouseEvent<HTMLElement>): void {
    const newUrlParams = Array.from(this.state.urlParams)

...
class UrlEncodeForm extends React.Component<{}, State> {
  ...
  onChangeDecodedTextArea (e: React.ChangeEvent<HTMLTextAreaElement>): void {
    this.setState({
      decodedText: e.target.value
    })
  }

  onChangeEncodedTextArea (e: React.ChangeEvent<HTMLTextAreaElement>): void {
    this.setState({
      encodedText: e.target.value
    })
  }

  onClickUrlEncode (e: React.MouseEvent<HTMLElement>): void {
    this.setState({
      encodedText: encodeURIComponent(this.state.decodedText)
    })
  }

  onClickUrlDecode (e: React.MouseEvent<HTMLElement>): void {
    try {
      this.setState({
      ...
      this.setState({
        decodedText: `can not decode. ${errMessage}.`
      })
    }
  }
}
